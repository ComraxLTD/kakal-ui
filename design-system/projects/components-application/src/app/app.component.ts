...
"city": {"label":"Xinsheng","value":60}
  ...
  // the tabs are the content of the external kkl-tabs
  tabs!: { key: string, label: string }[];

  // the innerTabs are the content of the kkl-tabs within the table
  innerTabs!: { key: string, label: string }[];

  // give a description to ech of them, of find a better way to handle it
  cityTab!: string
  nameTab!: string
  dobTab!: string
  occupationTab!: string
  yearsOfExperienceTab!: number

  constructor() { }

  ngOnInit(): void {
    // ComraxTablesService inserting data into dataSource
    this.tabs = [{ key: 'First', label: 'בדיקה' }, { key: 'Second', label: 'test' }, { key: 'Third', label: 'עמוד 3' }];
    this.innerTabs = [
      { key: 'name', label: 'שם' },
  ...
  rowActions: RowActionModel[] = [
    { type: 'inlineExpand', icon: 'expand', label: 'label' }
  ]

  // same
  onExpand(event: any) {
    this.nameTab = event.name
    this.occupationTab = event.occupation
    this.yearsOfExperienceTab = event.yearsOfExperience
    this.dobTab = event.dob
    this.cityTab = event.city.label
  }
